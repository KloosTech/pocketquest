...
/**
 * Bresenham's line algorithm — true from [from] to [to] iff no blocked tile
 * lies on the straight line between them (endpoints excluded, so a target
 * standing on a walkable tile is always reachable once LoS to that tile is
...
fun hasLineOfSight(from: GridPos, to: GridPos, map: BattleMap): Boolean {
    ...
    // A 1-step diagonal is blocked if either shared cardinal neighbour is a wall —
    ...
    if (abs(to.col - from.col) == 1 && abs(to.row - from.row) == 1) {
        if (!map.isWalkable(GridPos(to.col, from.row)) || !map.isWalkable(GridPos(from.col, to.row))) return false
    }
    ...
    while (true) {
        ...
        if (!map.isWalkable(GridPos(x, y))) return false
    }
    return true
}
